...
model_gzjx <- svyglm(
  sfyy ~ rcs(gzjx, 4) + RIDAGEYR + bmifd + yjfd + RIAGENDR + RIDRETH1 + 
...
model_zmjx <- svyglm(
  sfyy ~ rcs(zmjx, 4) + RIDAGEYR + bmifd + yjfd + RIAGENDR + RIDRETH1 + 
    DMDEDUC2 + DMDMARTZ + jtpir + gzqk + BPQ020 + tnbqk + xyqk + hdqkefl,
  design = design,
  family = quasibinomial()
)
...
model_pjjx <- svyglm(
  sfyy ~ rcs(pjjx, 4) + RIDAGEYR + bmifd + yjfd + RIAGENDR + RIDRETH1 + 
    DMDEDUC2 + DMDMARTZ + jtpir + gzqk + BPQ020 + tnbqk + xyqk + hdqkefl,
  design = design,
  family = quasibinomial()
)
