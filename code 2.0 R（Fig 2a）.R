...
data$gzrssjxsz <- as.numeric(data$gzrssjxsz)
data$zmrssjxsz <- as.numeric(data$zmrssjxsz)
data$pjrssjxsz <- as.numeric(data$pjrssjxsz)
...
time_vars <- c("gzrssjxsz", "zmrssjxsz", "pjrssjxsz")
...
for (var in time_vars) {
  ...
  test_results[[var]] <- tidy(chisq_test) %>%
    mutate(Variable = var) %>%
    select(Variable, Statistic = statistic, p_value = p.value)
}

all_test_results <- do.call(rbind, test_results)
rownames(all_test_results) <- NULL

print("Weighted depression probability by time period:")
...
write.csv(all_results, "weighted_depression_probability_by_time_period.csv", row.names = FALSE)
...
all_test_results$Bonferroni_adjusted_p_value <- bonferroni_adj_p
all_test_results$BH_adjusted_p_value <- bh_adj_p

print("Weighted depression probability by time period:")
print(all_results, row.names = FALSE)

print("Association tests between time variables and depression status (with multiple comparison adjustment):")
print(all_test_results, row.names = FALSE)
...
all_results <- all_results %>%
  mutate(Variable_Label = case_when(
    Variable == "gzrssjxsz" ~ "Workday sleep onset time",
    Variable == "zmrssjxsz" ~ "Weekend sleep onset time", 
    Variable == "pjrssjxsz" ~ "Average sleep onset time"
  ))

fill_colors <- c("Workday sleep onset time" = "sienna", 
                 "Weekend sleep onset time" = "mediumpurple", 
                 "Average sleep onset time" = "#888888")
...
ggplot(all_results, aes(x = as.factor(Time_Period), y = Depression_Probability * 100, fill = Variable_Label)) +
  ...
  labs(title = "Association between sleep onset time and depression prevalence in the NHANES (2017–2023)",
       x = "Hour (h)", 
