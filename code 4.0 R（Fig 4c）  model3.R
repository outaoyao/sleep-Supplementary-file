...
data$pjsmjl <- factor(data$pjsmjl, levels = c(0, 1, 2, 3), labels = c("No dysregulation", "Mild dysregulation", "Moderate dysregulation", "Severe dysregulation"))
...
design_obj <- svydesign(
  id = ~SDMVPSU,          # Cluster variable (primary sampling unit)
  strata = ~SDMVSTRA,     # Stratification variable
  weights = ~mecx,        # Weight variable
  data = data,
  nest = TRUE             # Usually set to TRUE, indicating PSUs are nested within strata
)
...
model2 <- svyglm(
  sfyy ~ pjsmjl + RIAGENDR + RIDAGEYR + RIDRETH1,
  design = design_obj,
...
all_results <- rbind(
  results1[grepl("pjsmjl", results1$Variable), ],
  results2[grepl("pjsmjl", results2$Variable), ],
  results3[grepl("pjsmjl", results3$Variable), ]
)
...
model3_data <- all_results[all_results$Model == "Model 3: Fully adjusted" & grepl("pjsmjl", all_results$Variable), ]

model3_data$Variable <- gsub("pjsmjl", "", model3_data$Variable)
...
ref_group$OR <- 1
ref_group$CI_lower <- 1
ref_group$CI_upper <- 1
ref_group$P_value <- ""
...
labeltext <- cbind(
  ...
  c("OR (95% CI)", 
    ifelse(model3_data$Variable == "MSRD-G 0", 
           "ref",
...
is_summary <- c(TRUE, FALSE, FALSE, FALSE, FALSE)
...
forestplot(labeltext = labeltext,
           ...
           xlog = TRUE,
           zero = 1,
           col = fpColors(box = "#6BAED6", lines = "#6BAED6", summary = "black"),
           boxsize = 0.25,
           lwd.ci = 2,
           ci.vertices = TRUE,
           title = "Sensitivity analysis: Association between 
           MSRD-G and depression risk 
           (Model 3)",
           xlab = "OR (95% CI)",
           txt_gp = fpTxtGp(
             xlab = gpar(cex = 1.1),
