...
model_gzzd <- svyglm(
  sfyy ~ rcs(gzzd, 4) + RIDAGEYR + bmifd + yjfd + RIAGENDR + RIDRETH1 + 
...
model_zmzd <- svyglm(
  sfyy ~ rcs(zmzd, 4) + RIDAGEYR + bmifd + yjfd + RIAGENDR + RIDRETH1 + 
    DMDEDUC2 + DMDMARTZ + jtpir + gzqk + BPQ020 + tnbqk + xyqk + hdqkefl,
  design = design,
  family = quasibinomial()
)
...
model_pjsmzd <- svyglm(
  sfyy ~ rcs(pjsmzd, 4) + RIDAGEYR + bmifd + yjfd + RIAGENDR + RIDRETH1 + 
    DMDEDUC2 + DMDMARTZ + jtpir + gzqk + BPQ020 + tnbqk + xyqk + hdqkefl,
  design = design,
  family = quasibinomial()
)
